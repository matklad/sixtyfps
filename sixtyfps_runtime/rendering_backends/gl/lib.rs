...
impl ItemRenderer for GLItemRenderer {
    ...
    fn draw_text(&mut self, pos: Point, text: std::pin::Pin<&sixtyfps_corelib::items::Text>) {
        ...
        self.canvas
            .borrow_mut()
            .fill_text(
                pos.x + sixtyfps_corelib::items::Text::FIELD_OFFSETS.x.apply_pin(text).get(),
                pos.y + sixtyfps_corelib::items::Text::FIELD_OFFSETS.y.apply_pin(text).get(),
                sixtyfps_corelib::items::Text::FIELD_OFFSETS.text.apply_pin(text).get().as_str(),
                paint,
            )
            .unwrap();
    }
