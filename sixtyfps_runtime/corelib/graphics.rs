...
pub trait HasFont {
    ...
    fn font(&self, window: &crate::eventloop::ComponentWindow) -> Option<ScaledFont> {
        window
            .0
            .font(self.font_request(window))
            .map(|font| ScaledFont { font, pixel_size: self.font_pixel_size(window) })
    }
}
